...
attendance_path = os.path.join(BASE_DIR, "Attendance(Manually)")
...
def manually_fill():
    global sb
    sb = tk.Tk()
    sb.iconbitmap(os.path.join(BASE_DIR, "AMS.ico"))
    ...
    def err_screen_for_subject():
        ...
        ec = tk.Tk()
        ec.geometry("300x100")
        ec.iconbitmap(os.path.join(BASE_DIR, "AMS.ico"))
        ec.title("Warning!!")
    ...
    def fill_attendance():
        ...
        if sub == "":
            ...
            sb.destroy()
            MFW = tk.Tk()
            MFW.iconbitmap(os.path.join(BASE_DIR, "AMS.ico"))
            MFW.title("Manually attendance of " + str(sub))
            ...
            def err_screen1():
                ...
                errsc2.iconbitmap(os.path.join(BASE_DIR, "AMS.ico"))
            ...
            def create_csv():
                df = pd.DataFrame(d).T # Transpose the DataFrame before saving to fix the structure.
                path = os.path.join(attendance_path, sub) # creating correct path
                if not os.path.exists(path): # Check if path exist and create if doesn't
                    os.makedirs(path)
                csv_name = os.path.join(path,f"{sub}_{Date}_{Hour}-{Minute}-{Second}.csv")
